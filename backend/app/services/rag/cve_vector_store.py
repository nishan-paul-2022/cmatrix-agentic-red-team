...
class CVEVectorStore:
    ...
    async def add_cves_batch(self, cves: List[CVEMetadata], batch_size: int = 100) -> Tuple[int, int]:
        """
    ...
    async def search(
        ...
        try:
            ...
            logger.info(f"CVE search completed in {search_time_ms:.2f}ms, found {len(results)} results")
            return response
    ...
    def _build_filters(
        ...
        if min_cvss_score is not None:
            must_conditions.append(
                models.FieldCondition(
                    key="cvss_base_score",
                    range=models.Range(gte=min_cvss_score)
                )
            )
        ...
        if max_cvss_score is not None:
            must_conditions.append(
                models.FieldCondition(
                    key="cvss_base_score",
                    range=models.Range(lte=max_cvss_score)
                )
            )
        ...
        if severity:
            must_conditions.append(
                models.FieldCondition(
                    key="severity",
                    match=models.MatchValue(value=severity.upper())
                )
            )
        ...
        if exploit_available is not None:
            must_conditions.append(
                models.FieldCondition(
                    ...
                    match=models.MatchValue(value=exploit_available)
                )
            )
