...
- **Repo root contains**: `app-backend/`, `app-frontend/`, `docs/`, `paper-research/`, `assets/`, `docker-compose*.yml`, `docker.sh`, `install.sh`, `Makefile`, `package.json`, `llm-config-template.json`, plus dotfolders (`.github/`, `.husky/`, `.vscode/`, `.git/`) and `node_modules/`.
- **Discrepancy flagged**: the user prompt references `paper-04-agent-reasoning/` at repo root; in this repository it is located at `paper-research/paper-04-agent-reasoning/`.
- **LaTeX build system**: root `Makefile` builds papers via `make paper-03`, using `paper-research/paper-04-agent-reasoning/content/main.tex` and emitting `paper-research/paper-04-agent-reasoning/paper.pdf`.
...
- **Research build tooling**: `latexmk` via root `Makefile` for papers under `paper-research/`.
...
- `paper-research/paper-04-agent-reasoning/content/main.tex`: current LaTeX draft (note: content is presently framed around “LLMOrch-VAPT” and uses raster `architecture.png`, `routing-flow.png`, `safety-gate.png`, `eval-graph.png`).
- `paper-research/paper-04-agent-reasoning/assets/`: contains raster figures referenced by `content/main.tex`.
- `paper-research/paper-02-red-teaming/`, `paper-research/paper-03-hitl-safety/`, etc.: other paper tracks referenced by the root README and build system.
...
- **Path mismatch**: prompt references `paper-04-agent-reasoning/` at root; repository uses `paper-research/paper-04-agent-reasoning/`.
- **Paper-03 draft mismatch with topic**: `paper-research/paper-04-agent-reasoning/content/main.tex` is currently titled around **“LLMOrch-VAPT”** and includes content (e.g., “97.4% success rate across 1,500 tasks”, references to specific provider tiers and GPU setup) that is not yet validated here in Step 1a; this is a risk for later steps (Step 1b/1c).
- **Research asset format mismatch with required rules**: current paper assets are raster `.png` files in `paper-research/paper-04-agent-reasoning/assets/`, while the target process requires **TikZ/pgfplots** assets as separate `.tex` files. (This is not fixed in Step 1a; only flagged.)
...
- `paper-research/prompt.md`
- `paper-research/paper-04-agent-reasoning/content/main.tex`
- `paper-research/paper-04-agent-reasoning/discussion.md`
...
- **Paper directory resolution**: treated `paper-research/paper-04-agent-reasoning/` as the target paper workspace since repo root does not contain `paper-04-agent-reasoning/`.
...
- **Paper/implementation alignment**: `paper-research/paper-04-agent-reasoning/content/main.tex` currently includes strong quantitative claims (e.g., “1,500 tasks”, “97.4% success”, “80% cost reduction”) and multiple raster figures; Step 1b/1c must verify which are supported by code, logs, or datasets in this repo.
...
- **Target paper to read**: `paper-research/paper-02-red-teaming/` (per the process) and its contents, using this Step 1a artifact as the system ground truth.
- **Cross-check focus**:
...
- `paper-research/paper-04-agent-reasoning/assets/ASSET-INDEX.md`: Initialized asset index (empty tables; no figures/tables created in Step 1a by design).
