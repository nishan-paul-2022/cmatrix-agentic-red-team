...
|_L_2—Medium|0.4_≤_score_<_0.7|Moderate attack complexity (AC = Medium)<br>May involve limited user interaction (UI = Required)<br>Low privileges required (PR = Low)<br>Multi-step exploitation (ES = 3–5 steps)<br>PoCs require adaptation or chaining with reconnaissance<br>Examples: File upload + WebShell, SQL injection with login bypass High attack complexity (AC = High)<br>User interaction often required (UI = Required)|
