...
- [2.1. Pre-LLM automation and RL](#2-1-pre-llm-automation-and-rl)
- [2.2. LLM single-few-agent pipelines](#2-2-llm-single-few-agent-pipelines)
- [2.3. LLM multi-agent orchestration](#2-3-llm-multi-agent-orchestration)
- [2.4. Focused exploit studies (one-day, zero-day) and CTF-style agents.](#2-4-focused-exploit-studies-one-day-zero-day-and-ctf-style-agents)
- [2.5. Benchmarks and methodology](#2-5-benchmarks-and-methodology)
- [2.6. Positioning of our work](#2-6-positioning-of-our-work)
- [2.7. Takeaways.](#2-7-takeaways)
- [3. Background](#3-background)
- [3.1. Automated Pentest](#3-1-automated-pentest)
- [3.2. Multi-Agent AI Systems](#3-2-multi-agent-ai-systems)
- [3.3. LLM-based O ff ensive Agents](#3-3-llm-based-o-ff-ensive-agents)
- [g : C �→ A](#g-c-a)
- [3.4. Fine-tuning Methods for O ff ensive LLMs](#3-4-fine-tuning-methods-for-o-ff-ensive-llms)
- [4. Methodology](#4-methodology)
- [4.1. Overview of the proposed framework](#4-1-overview-of-the-proposed-framework)
- [4.2. Role Specialization](#4-2-role-specialization)
- [4.3. Task Coordination and Reflection](#4-3-task-coordination-and-reflection)
- [4.3.1. Task Coordination Graph](#4-3-1-task-coordination-graph)
- [4.3.2. Check and Reflection Mechanism](#4-3-2-check-and-reflection-mechanism)
- [4.4. Inter-Agent Communication Mechanism](#4-4-inter-agent-communication-mechanism)
- [4.5. Generative Behavior and Execution](#4-5-generative-behavior-and-execution)
- [5. Implementation, Benchmark Dataset and Metrics](#5-implementation-benchmark-dataset-and-metrics)
- [5.1. Experimental Setup](#5-1-experimental-setup)
- [5.1.1. Attacker Environment](#5-1-1-attacker-environment)
- [5.1.2. Victim Environment](#5-1-2-victim-environment)
- [5.1.3. Evaluation Protocol](#5-1-3-evaluation-protocol)
- [5.2. LLM models](#5-2-llm-models)
- [5.2.1. Evaluated Baseline Models](#5-2-1-evaluated-baseline-models)
- [5.2.2. Fine-Tuning Methodology](#5-2-2-fine-tuning-methodology)
- [5.2.3. Prompt Setting](#5-2-3-prompt-setting)
- [5.3. Benchmark Datasets](#5-3-benchmark-datasets)
- [5.4. Evaluation Metrics](#5-4-evaluation-metrics)
- [5.4.1. Overall Task Completion Rate](#5-4-1-overall-task-completion-rate)
- [5.4.2. Sub-task Completion Rate (1 Experiment)](#5-4-2-sub-task-completion-rate-1-experiment)
- [5.4.3. Sub-task Completion Rate (5 Experiments)](#5-4-3-sub-task-completion-rate-5-experiments)
- [6. Evaluation and results](#6-evaluation-and-results)
- [6.1. Experiment Scenarios](#6-1-experiment-scenarios)
- [6.2. Task Completion Performance Across pentest Categories](#6-2-task-completion-performance-across-pentest-categories)
- [6.2.1. Overall Task Completion Performance](#6-2-1-overall-task-completion-performance)
- [6.2.2. Sub-task Completion Performance (1 Experiment)](#6-2-2-sub-task-completion-performance-1-experiment)
- [6.2.3. Sub-task Completion Performance (5 Experiments)](#6-2-3-sub-task-completion-performance-5-experiments)
- [6.2.4. Comparative Insights](#6-2-4-comparative-insights)
- [6.3. Evaluation on Complex Real-World Exploitation Chains](#6-3-evaluation-on-complex-real-world-exploitation-chains)
- [6.3.1. Performance without RAG (No-RAG)](#6-3-1-performance-without-rag-no-rag)
- [6.3.2. Performance with RAG (RAG)](#6-3-2-performance-with-rag-rag)
- [7. Threats to Validity](#7-threats-to-validity)
- [7.1. Internal validity](#7-1-internal-validity)
- [7.2. External validity](#7-2-external-validity)
- [7.3. Construct validity](#7-3-construct-validity)
- [7.4. Reliability](#7-4-reliability)
- [8. Ethical Considerations and Responsible Use](#8-ethical-considerations-and-responsible-use)
...
> **Section Summary:** Pentest remains one of the most effective ways to assess the real-world security posture of modern information systems.
...
> **Section Summary:** PentestAgent adopts RAG-grounded, role-based collaboration (reconnaissance, triage, exploitation) to reduce hallucinations and improve next step selection [8].
...
> **Section Summary:** Outcomes hinge on (i) _grounding quality_ (RAG/summaries/validators) and (ii) _orchestration discipline_ (roles/PTG/reflection).


Outcomes hinge on (i) _grounding quality_ (RAG/summaries/validators) and (ii) _orchestration discipline_ (roles/PTG/reflection). Single-agent pipelines set baselines:
- role-structured multiagent systems consistently improve reliability
- reflective and IP-to-shell variants further push autonomy at kill-chain ends
- one-day/zero-day studies quantify limits of discovery vs. exploitation [22, 23]. Our approach emphasizes cost-effective, reproducible deployment with mid-scale open models, PTG structure, and grey-box prompts under open, reproducible protocols [10, 11, 26, 27].
...
> **Section Summary:** Pentest aims to evaluate the security of a target system by simulating adversarial behavior across phases such as reconnaissance, vulnerability enumeration, exploitation, and privilege escalation.
...
> **Section Summary:** Multi-Agent Systems (MAS) provide a natural architecture for automated pentest by assigning each pentest phase to a specialized agent.
...
> **Section Summary:** Within MAS, LLMs provide reasoning, contextual understanding, and code synthesis capabilities that align well with pentest workflows.
...
> **Section Summary:** Although proprietary LLMs, such as GPT-4 and Claude, offer strong performance, they introduce limitations in cost, reproducibility, and security control.
...
> **Section Summary:** To navigate the complexity of pentest, xOffense employs a role specialization strategy, mitigating the risk of information overload and ensuring contextual coherence across phases.
...
> **Section Summary:** The TCG and its integrated _Check and Reflection Mechanism_ form the cornerstone of xOffense’s penetration path planning, enabling systematic task execution and adaptive plan refinement.
...
> **Section Summary:** The TCG is a structured acyclic digraph, defined as _G_ = ( _V_ , _E_ ), where _V_ represents individual tasks and _E_ denotes dependencies, ensuring logical and conflict-free execution.
...
> **Section Summary:** The generation of erroneous commands and the lack of effective error-handling mechanisms pose significant challenges to LLM-based pentest.
...
> **Section Summary:** xOffense supports three operational modes, specifically automatic, semi-automatic, and manual, with the automatic mode
...
> **Section Summary:** All pentest experiments are executed from a dedicated attacker machine configured as a VMware virtual workstation
...
> **Section Summary:** Two types of target environments are deployed corresponding to the benchmark datasets:
...
> **Section Summary:** Within the unified environment described in Section 5.1.1, xOffense utilizes a core toolchain with specific versions detailed in Table 2.
...
> **Section Summary:** The fine-tuned model, namely Qwen3-32B-finetune is hosted on a dedicated compute node equipped with an NVIDIA A100 GPU having 80 GB VRAM.
...
> **Section Summary:** The experimental evaluation utilizes a broad spectrum of large language models to establish a rigorous comparative baseline.
...
> **Section Summary:** Unlike conventional large-scale model adaptations, which demand significant computational overhead, _xO_ ff _ense_ leverages LoRA to achieve domain specialization.
...
> **Section Summary:** To ensure a rigorous and practically relevant evaluation, we selected two complementary benchmarks that cover both synthetic vulnerabilities and realistic multi-stage exploitation scenarios.
...
> **Section Summary:** To evaluate the performance of _xO_ ff _ense_ , we employ three complementary metrics that capture both high-level task success and fine-grained sub-task robustness.
...
> **Section Summary:** This metric measures the percentage of target machines successfully compromised like the obtained flag within the allowed interaction budget.
...
> **Section Summary:** To gain insight into intermediate stages of pentest, we evaluate sub-task success rates, including service enumeration and vulnerability detection.
...
> **Section Summary:** To measure robustness and consistency, we also compute the cumulative completion rate across all five runs.
...
> **Section Summary:** We design five experimental scenarios to comprehensively evaluate _xO_ ff _ense_ across synthetic and real-world settings:
...
> **Section Summary:** Table 3 presents the overall task completion rates across all evaluated models on the AutoPenBench dataset.
...
> **Section Summary:** To assess finer-grained capabilities, we evaluated sub-task completion in a single-run experiment (Table 4).
...
> **Section Summary:** To evaluate robustness and stability, we conducted aggregated experiments over five runs (Table 5).
...
> **Section Summary:** A critical observation from these experiments is the disproportionate performance leap achieved through fine-tuning relative to model size.
...
> **Section Summary:** When augmenting the evaluation with the Knowledge Repository module, a substantial shift in performance trends emerges across the six real-world exploitation targets _(see Fig.
...
> **Section Summary:** The fine-tuning of Qwen3-32B on a CoT-enriched pentest dataset introduces potential internal threats.
...
> **Section Summary:** The evaluation settings of AutoPenBench and AI-PentestBenchmark, which approximate realistic penetration workflows, yet cannot fully capture the heterogeneity of productionscale environments.
...
> **Section Summary:** Task completion rate and exploitation success were employed as primary evaluation metrics.
...
> **Section Summary:** The reproducibility of results may be affected by stochastic factors inherent in both large language model inference and auxiliary system tools.
...
> **Section Summary:** This work addresses autonomous penetration testing, a domain with inherent dual-use implications.
...
- [9] He Kong, Die Hu, Jingguo Ge, Liangxiong Li, Tong Li, and Bingzhen Wu. VulnBot: Autonomous penetration testing for a multi-agent collaborative framework. _
...
- [12] An Yang, Anfeng Li, Baosong Yang, Beichen Zhang, Binyuan Hui, Bo Zheng, Bowen Yu, Chang Gao, Chengen Huang, Chenxu Lv, et al. Qwen3 technical report. _
...
- [18] Jiacen Xu, Jack W Stokes, Geoff McDonald, Xuesong Bai, David Marshall, Siyue Wang, Adith Swaminathan, and Zhou Li. AutoAttacker: A large language model guided system to implement automatic cyberattacks. _

- [19] Hanzheng Dai, Yuanliang Li, Zhibo Zhang, and Jun Yan. Refpentester: A knowledge-informed self-reflective penetration testing framework based on large language models. _

- [20] Sho Nakatani. Rapidpen: Fully automated ip-to-shell penetration testing with llm-based agents. _
...
- [22] Richard Fang, Rohan Bindu, Akul Gupta, and Daniel Kang. Llm agents can autonomously exploit one-day vulnerabilities. _

- [23] Yuxuan Zhu, Antony Kellermann, Akul Gupta, Philip Li, Richard Fang, Rohan Bindu, and Daniel Kang. Teams of llm agents can exploit zero-day vulnerabilities. _

- [24] Lajos Muzsai, David Imolai, and Andr´as Luk´acs. Hacksynth: Llm agent and evaluation framework for autonomous penetration testing. _
...
written in go. `https://github.com/OJ/gobuster` , 2025.
   ...
   - [40] Hongli Yu, Tinghong Chen, Jiangtao Feng, Jiangjie Chen, Weinan Dai, Qiying Yu, Ya-Qin Zhang, Wei-Ying Ma, Jingjing Liu, Mingxuan Wang, et al. Memagent: Reshaping long-context llm with multi-conv rl-based memory agent. _
...
- [26] Yuxuan Zhu, Antony Kellermann, Dylan Bowman, Philip Li, Akul Gupta, Adarsh Danda, Richard Fang, Conner Jensen, Eric Ihli, Jason Benn, Jet Geronimo, Avi Dhir, Sudhit Rao, Kaicheng Yu, Twm Stone, and Daniel Kang. Cve-bench: A benchmark for ai agents’ ability to exploit realworld web application vulnerabilities. _
...
- [28] Julius Henke. Autopentest: Enhancing vulnerability management with autonomous llm agents. _
