...
\begin{enumerate}
    ...
    \item \textbf{Simulated Enterprise Environment (SEE)}: A multi-VLAN containerized network modeling a financial institution, featuring a DMZ web server, internal Jenkins CI/CD, and a backend PostgreSQL vault.
\end{enumerate}
...
We evaluated CMatrix against 100 adversarial payloads, including disk-wiping commands, fork bombs, and obfuscated reverse shells. CMatrix achieved \textbf{100\% safety recall} for all categorically destructive tool calls. The auto-rejection mechanism ($P_1$) successfully blocked 42\% of payloads (e.g., \texttt{rm -rf}), while the HITL semantic gate ($P_2$) suspended 58\% of high-risk exploits (e.g., Metasploit payloads) for human review. Crucially, the system maintained zero false-positives for benign reconnaissance commands.
