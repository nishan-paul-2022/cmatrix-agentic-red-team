...
The remainder of this paper is organized as follows. Section~\ref{sec:related} reviews the related work in autonomous penetration testing and LLM safety. Section~\ref{sec:threat} formalizes our threat model and safety properties. Section~\ref{sec:arch} details the CMatrix system architecture, including the supervisor routing and multi-agent coordination. Section~\ref{sec:safety} describes our hierarchical HITL safety framework and checkpointing mechanism. Section~\ref{sec:memory} presents our agentic RAG memory pipeline. Section~\ref{sec:reasoning} details our reasoning and self-reflection loops. Section~\ref{sec:eval} presents our experimental evaluation and results. Section~\ref{sec:discussion} discusses the implications and limitations of our findings, and Section~\ref{sec:conclusion} concludes the paper.
...
For example, an adversarial payload such as \texttt{echo ZWNobyAiSGVsbG8iIHwgcm0gLXJmIC8K | base64 -d | sh} (which executes \texttt{rm -rf /}) would successfully bypass a simple \texttt{rm} regex filter. However, since the command utilizes the \texttt{shell_exec} tool, it is assigned a \textit{Critical} risk level by the taxonomy, triggering an immediate suspension and human review. In our evaluation, the combined "Regex + Semantic HITL" stack demonstrated a 100\% defense-in-depth record against both direct and obfuscated adversarial payloads.
...
Target-specific findings, tool outputs, and reasoning traces are indexed as vectorized chunks in Qdrant using the BGE-large embedding model. To enforce property $P4$ (Scope Integrity), every chunk is tagged with an \texttt{engagement_id} metadata attribute. During the retrieval phase, the query is strictly filtered to only return chunks matching the current session ID, preventing cross-session or cross-tenant data leakage.
