...
% Fix for Roman numerals in section numbering
\renewcommand{\thesection}{\Roman{section}}
\renewcommand{\thesubsection}{\Alph{subsection}}

% Fix for Figure prefix (ensuring Fig. 1. format)
\usepackage[labelsep=period]{caption}
\DeclareCaptionLabelFormat{figprefix}{Fig. #2}
\captionsetup[figure]{labelformat=figprefix}

% Fix for Table label (ensuring TABLE I format)
\DeclareCaptionLabelFormat{tableprefix}{TABLE #2}
\captionsetup[table]{labelformat=tableprefix, justification=centering, font=sc}

% Rename Listings to Figures as per review recommendation
...
\begin{lstlisting}[language=Python, label=lst:ollama, caption=Detailed Provider Implementation for Ollama]
...
To better understand the performance gap, we analyzed specific failure modes. Fig. \ref{lst:exploit} shows a typical case where a "Flash" tier model failed to identify a potential blind SQL injection, whereas the "Reasoning" tier model successfully mapped out the exploit chain.

\begin{lstlisting}[language=bash, label=lst:exploit, caption=Exploit Chain Reasoning (Reasoning Tier)]
