#!/usr/bin/env python3
"""
build_ppt.py — CMatrix Presentation Builder
============================================
Usage:
    python3 build_ppt.py              # builds to default output path
    python3 build_ppt.py out.pptx     # builds to custom path
...
Adding a slide:   create slide_XX_name.py with a build_slide(prs) function.
...
OUTPUT_PATH = sys.argv[1] if len(sys.argv) > 1 else os.path.join(SCRIPT_DIR, "CMatrix_Presentation.pptx")
...
def discover_slides(directory: str) -> list[str]:
    """Return sorted list of slide_*.py file paths."""
